...
func TestDB_Put(t *testing.T) {
	...
	// 5.写到数据文件进行了转换
	// ？？ 报错panic: remove C:\Users\40466\AppData\Local\Temp\bitcask-go-put2298598267\000000000.data: The process cannot access the file because it is being used by another process.
	for i := 0; i < 1000000; i++ {
...
func TestDB_Get(t *testing.T) {
	...
	opts.DataFileSize = 64 * 1024 * 1024
	db, err := Open(opts)
	// ？？ 同样，加上 destroy(db) 则报错panic: remove C:\Users\40466\AppData\Local\Temp\bitcask-go-put2298598267\000000000.data: The process cannot access the file because it is being used by another process.
	// defer destroyDB(db)
	assert.Nil(t, err)
	assert.NotNil(t, db)
